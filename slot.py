...
# Store latest events for real-time display
latest_exit_event = {'plate': None, 'slot': None, 'timestamp': None}
latest_detection_event = {'plate': None, 'timestamp': None}
...
def allocation():
    ...
    try:
        free_slots = Slot.query.filter_by(status='free').count()
    except Exception:
        free_slots = 0
...
def latest_exit():
    ...
    try:
        if latest_exit_event['plate']:
            ...
            return jsonify({
                'success': True,
                'slot_number': f"P{event['slot']:02d}" if isinstance(event['slot'], int) else event['slot'],
                'plate': event['plate'],
...
@app.route('/api/latest-detection', methods=['GET'])
def latest_detection():
    """Get the latest vehicle detection event (pre-allocation)"""
    global latest_detection_event
    try:
        if latest_detection_event['plate']:
            event = latest_detection_event.copy()
            # Reset after sending
            latest_detection_event = {'plate': None, 'timestamp': None}
            return jsonify({
                'success': True,
                'plate': event['plate'],
                'message': 'Vehicle detected, processing...'
            }), 200
        else:
            return jsonify({'success': False, 'message': 'No recent detections'}), 200
    except Exception as e:
        logger.error(f'[LATEST_DETECTION] Error: {e}')
        return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/api/notify-detection', methods=['POST'])
def notify_detection():
    """Notify the API that a vehicle has been detected"""
    global latest_detection_event
    try:
        ...
        logger.info(f'[NOTIFY] Vehicle detected: {plate}')
        latest_detection_event = {
            'plate': plate,
            'timestamp': datetime.now().isoformat()
        }
        return jsonify({'success': True, 'message': 'Detection registered'}), 200
    except Exception as e:
...
def latest_allocation():
    ...
    try:
        ...
        if latest_txn and latest_txn.slot:
            return jsonify({
                'success': True,
                'slot_number': f"P{latest_txn.slot.number:02d}",
...
def entry_vehicle_internal(plate):
    ...
    try:
        ...
        return {
            'success': True,
            'action': 'entry',
            'message': f'Vehicle {plate} ENTERED. Slot P{available_slot.number:02d} assigned.',
            'slot_id': available_slot.id,
            'slot_number': f"P{available_slot.number:02d}",
            'txn_id': txn.id
        }
    except Exception as e:
        db.session.rollback()
...
def exit_vehicle_internal(plate, txn):
    ...
    try:
        ...
        return {
            'success': True,
            'action': 'exit',
            'message': f'Vehicle {plate} EXITED from Slot {f"P{txn.slot.number:02d}" if txn.slot else "N/A"}',
            'duration_minutes': duration_minutes,
            'charge': txn.charge,
            'payment_status': 'pending',
            'slot_number': f"P{txn.slot.number:02d}" if txn.slot else None
        }
    except Exception as e:
        db.session.rollback()
...
def detect_plate():
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'action': 'ENTRY',
            'slot_number': f"P{free_slot.number:02d}",
            'plate': plate,
            'transaction_id': txn.id,
            'message': f'Slot P{free_slot.number:02d} allocated',
            'allocation_url': f'/allocation?slot=P{free_slot.number:02d}&plate={plate}&txn={txn.id}'
        }), 200
...
def exit_vehicle():
    ...
    try:
        ...
        logger.info(f'[EXIT] Processing detection-based exit for plate: {plate}')
        
        # Find the active transaction (Still inside OR at exit waiting for payment)
        txn = Transaction.query.filter(
            Transaction.plate == plate,
            (Transaction.time_out == None) | (Transaction.payment_status == 'pending')
        ).order_by(Transaction.time_in.desc()).first()
        ...
        # Use internal function to process exit
...
def manual_entry():
    ...
    try:
        # Check for vehicle already inside or at exit waiting for payment
...
def manual_exit():
    ...
    try:
        # Find active transaction (Parked or at exit waiting for payment)
        txn = Transaction.query.filter(
            Transaction.plate == plate,
            (Transaction.time_out == None) | (Transaction.payment_status == 'pending')
        ).order_by(Transaction.time_in.desc()).first()
    ...
    try:
        ...
        for txn in txns:
            vehicle_detail = {
                'id': txn.id,
                'plate': txn.plate,
                'slot_number': f"P{txn.slot.number:02d}" if txn.slot else 'N/A',
